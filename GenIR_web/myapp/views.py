...
from .models import TaskData, DialogueAnnotation, LinkScore
...
def perplexity_api(request):
    if request.method == "POST":
        try:
            ...
            headers = {
                "Authorization": "Bearer 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",  # 替换为你的 API 密钥
...
class TaskDetailView(DetailView):
    ...
    http_method_names = ['get', 'post']  # 允许 GET 和 POST 请求
    ...
    def get_context_data(self, **kwargs):
        ...
        for click in task.behavior.get('clicks', []):
            if click.get('elementType') == 'link':
                dialogue_index = click['dialogueContext']['dialogueIndex']
                link_href = click['href']
                link_key = (dialogue_index, link_href)

                print(f"处理链接: {link_href}, 对话索引: {dialogue_index}")  # 调试信息

                # 如果该链接没有被添加过，才加入到 filtered_clicks
                if link_key not in unique_clicks:
                    unique_clicks.add(link_key)
                    filtered_clicks.append(click)

        print(f"共计处理了 {len(filtered_clicks)} 个唯一链接。")  # 调试信息
        ...
        try:
            ...
            for dialogue in task.dialogue:
                ...
                for response in dialogue['responses']:
                    ...
                    quality_key = f'quality_score_q_{query_id}_r_{response_id}'
                    quality_score = request.POST.get(quality_key)
                    comment_key = f'annotation_comment_q_{query_id}_r_{response_id}'
                    annotation_comment = request.POST.get(comment_key)

                    DialogueAnnotation.objects.update_or_create(
                        task=task,
            ...
            for click in task.behavior.get('clicks', []):
                if click.get('elementType') == 'link':
                    score_key = f"link_score_{click['dialogueContext']['linkIndexInResponse']}"
                    link_score = request.POST.get(score_key)
                    if link_score is not None:  # 确保有评分值才进行保存
                        LinkScore.objects.update_or_create(
                            task=task,
                            link_index=click['dialogueContext']['linkIndexInResponse'],
                            annotator=request.user,  # 添加 annotator 到查询条件
                            defaults={
                                'score': link_score,
                                # annotator 不需要在 defaults 中，因为已经在查询条件里
                            }
                        )
